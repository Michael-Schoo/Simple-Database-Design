...
for i in range(10):
    ...
    image_id = None
    if image_url:
        con.execute("INSERT INTO Images (image_url, alt) VALUES (?, ?)", (image_url, "Album art for "+title))
        image_id = con.execute("SELECT last_insert_rowid()").fetchone()[0]

    con.execute("INSERT INTO MusicMetadata (title, artist, image_id) VALUES (?, ?, ?)", (title, artist, image_id))
